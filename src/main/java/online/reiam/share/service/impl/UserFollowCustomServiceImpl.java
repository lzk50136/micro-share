...
public class UserFollowCustomServiceImpl implements UserFollowCustomService {
    ...
    public UserFollow getFollow(Integer userId, Integer followId) {
        ...
        return userFollowService.getOne(queryWrapper2);
    }
    ...
    public void follow(Integer userId, Integer followId, UserFollowRequest userFollowRequest) {
        ...
        if (userFollowRequest.getFollow()) {
            ...
            if (userFollow == null) {
                ...
                userFollowService.save(userFollow2);
            } else {
                // 如果已经是关注状态，无需操作
                if (userFollow.getFollow()) {
                    ...
                    userFollow2.setId(userFollow.getId())
                            ...
                            .setModifiedTime(LocalDateTime.now());
                    userFollowService.updateById(userFollow2);
                }
            }
            // 查找对方是否有关注自己
            ...
            if (userFollow3 != null && userFollow3.getFollow()) {
                // 修改自己互粉状态
                UserFollow userFollow4 = userFollowService.getById(userFollow2.getId());
                UserFollow userFollow5 = new UserFollow();
                userFollow5.setId(userFollow4.getId())
                        .setEachOther(true)
                        .setVersion(userFollow4.getVersion())
                        .setModifiedTime(LocalDateTime.now());
                userFollowService.updateById(userFollow5);
                // 修改对方互粉状态
                ...
                userFollow6.setId(userFollow3.getId())
                        ...
                        .setModifiedTime(LocalDateTime.now());
                userFollowService.updateById(userFollow6);
            }
            // 自己关注的人数+1
            ...
            if (userFollow.getEachOther()) {
                // 修改自己互粉状态
                UserFollow userFollow2 = userFollowService.getById(userFollow.getId());
                UserFollow userFollow3 = new UserFollow();
                userFollow3.setId(userFollow.getId())
                        .setEachOther(false)
                        .setVersion(userFollow2.getVersion())
                        .setModifiedTime(LocalDateTime.now());
                userFollowService.updateById(userFollow3);
                // 修改对方互粉状态
